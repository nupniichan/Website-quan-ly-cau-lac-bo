...
const ManageClubs = () => {
  ...
  const handleAddClub = async () => {
    ...
    try {
      ...
      if (error.response) {
        alert(`Lỗi khi thêm câu lạc bộ: ${error.response.data.message || 'Không xác định'}`);
      } else {
  ...
  const handleUpdateClub = async () => {
    try {
      ...
      if (error.response) {
        alert(`Lỗi khi cập nhật câu lạc bộ: ${error.response.data.message || 'Không xác định'}`);
      } else {
        alert('Không thể kết nối đến server. Vui lòng thử lại sau.');
      }
    }
  ...
  const openEditDialog = async (id) => {
    try {
      ...
      if (error.response) {
        alert(`Lỗi khi lấy thông tin câu lạc bộ: ${error.response.data.message || 'Không xác định'}`);
      } else {
        alert('Không thể kết nối đến server. Vui lòng thử lại sau.');
      }
    }
  ...
  return (
    <div className="mt-12 mb-8 flex flex-col gap-12">
      ...
      <Dialog open={isDialogOpen} handler={() => setIsDialogOpen(false)} size="xl">
        <DialogHeader>{editingClubId ? "Chỉnh sửa Câu Lạc Bộ" : "Thêm Câu Lạc Bộ Mới"}</DialogHeader>
        <DialogBody divider className="grid grid-cols-2 gap-4">
          <Input label="Tên CLB" value={newClub.ten} onChange={(e) => setNewClub({ ...newClub, ten: e.target.value })} />
        ...
        <DialogFooter>
          ...
          <Button variant="gradient" color="green" onClick={editingClubId ? handleUpdateClub : handleAddClub}>
