...
const BudgetAllocation = () => {
  ...
  const handleAddAllocation = async () => {
    try {
      ...
      alert(`Lỗi khi thêm phân bổ ngân sách: ${error.response?.data?.message || 'Không xác định'}`);
    }
  };
  ...
  const handleUpdateAllocation = async () => {
    try {
      ...
      alert(`Lỗi khi cập nhật phân bổ ngân sách: ${error.response?.data?.message || 'Không xác định'}`);
    }
  };
  ...
  const handleDeleteAllocation = async (id) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa phân bổ ngân sách này?")) {
      try {
        ...
        alert(`Lỗi khi xóa phân bổ ngân sách: ${error.response?.data?.message || 'Không xác định'}`);
      }
    }
  };
  ...
  const openEditDialog = (id) => {
    const allocationToEdit = allocations.find(allocation => allocation._id === id);
    if (allocationToEdit) {
      setNewAllocation({
        ...allocationToEdit,
  ...
  return (
    <div className="mt-12 mb-8 flex flex-col gap-12">
      <Card>
        ...
        <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
          <div className="flex justify-end mb-4 px-6">
            ...
            >
              <PlusIcon strokeWidth={2} className="h-4 w-4" /> Thêm phân bổ
            </Button>
          </div>
          {isLoading ? (
            ...
            <table className="w-full min-w-[640px] table-auto">
              <thead>
                <tr>
                  {["Câu lạc bộ", "Số tiền", "Mục đích", "Ngày phân bổ", "Thao tác"].map((el) => (
              ...
              <tbody>
                {allocations.map(({ _id, club, amount, purpose, allocationDate }, key) => {
                  const className = `py-3 px-5 ${
                    key === allocations.length - 1 ? "" : "border-b border-blue-gray-50"
                  }`;
                    ...
                    <tr key={_id}>
                      ...
                      <td className={className}>
                        <div className="flex items-center gap-2">
                          ...
                          <Button size="sm" color="red" className="flex items-center gap-2" onClick={() => handleDeleteAllocation(_id)}>
      ...
      <Dialog open={isDialogOpen} handler={() => setIsDialogOpen(false)} size="xl">
        <DialogHeader>{editingAllocationId ? "Ch���nh sửa Phân bổ Ngân sách" : "Thêm Phân bổ Ngân sách Mới"}</DialogHeader>
        <DialogBody divider className="grid grid-cols-2 gap-4">
          <Select 
            label="Câu lạc bộ" 
            value={newAllocation.club ? newAllocation.club.toString() : ''} 
            onChange={(value) => setNewAllocation({ ...newAllocation, club: value })}
          >
            {clubs.map((club) => (
              <Option key={club._id} value={club._id.toString()}>{club.ten}</Option>
            ))}
          </Select>
          <Input type="number" label="Số tiền" value={newAllocation.amount} onChange={(e) => setNewAllocation({ ...newAllocation, amount: e.target.value })} />
          <Input label="Mục đích" value={newAllocation.purpose} onChange={(e) => setNewAllocation({ ...newAllocation, purpose: e.target.value })} />
          <Input type="date" label="Ngày phân bổ" value={newAllocation.allocationDate} onChange={(e) => setNewAllocation({ ...newAllocation, allocationDate: e.target.value })} />
        </DialogBody>
        <DialogFooter>
          <Button variant="text" color="red" onClick={() => setIsDialogOpen(false)} className="mr-1">
            Hủy
          </Button>
          <Button variant="gradient" color="green" onClick={editingAllocationId ? handleUpdateAllocation : handleAddAllocation}>
