...
const ManageMembers = () => {
    ...
    const handleAddMember = async () => {
        try {
            ...
            // );
            notification.error({message: "Lỗi khi thêm thành viên", description: error.message || "Không xác định"});
        }
    };
    ...
    const handleUpdateMember = async () => {
        ...
        try {
            ...
            //     }`,
            // );
            notification.error({message: "Lỗi khi cập nhật thành viên", description: error.response?.data?.message || "Không xác định"});
        }
    };
    ...
    const handleDeleteMember = async (maSoHocSinh, tinhTrang) => {
        ...
        if (tinhTrang === "Đang hoạt động") {
            ...
            message.warning({content: "Không thể xóa thành viên đang hoạt động!"});
            return;
        }
        ...
        if (window.confirm("Bạn có chắc chắn muốn xóa thành viên này?")) {
            try {
                ...
                //     }`,
                // );
                notification.error({message: "Lỗi khi xóa thành viên", description: error.response?.data?.message || "Không xác định"});
            }
        }
    };
    ...
    const openEditDialog = async (maSoHocSinh) => {
        try {
            ...
            //     }`,
            // );
            notification.error({message: "Lỗi khi lấy thông tin thành viên", description: error.response?.data?.message || "Không xác định"});
        }
    };
    ...
    const filteredMembers = useMemo(() => {
        const filtered = members.filter((member) => {
            ...
            if (/^\d+$/.test(searchTerm)) {
                return member.maSoHocSinh.toLowerCase().includes(
                    searchTerm.toLowerCase(),
                );
            }

            // Nếu không phải MSSV, tìm theo tên hoặc lớp
            const nameOrClassMatch =
            ...
            return nameOrClassMatch && genderMatch && roleMatch && dateMatch;
        });
